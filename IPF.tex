\documentclass[aip,pof,nofootinbib,reprint,onecolumn,draft]{revtex4-1}
%\documentclass[11pt,openany,onecolumn,a4wide,pof]{article}
...
\newcommand{\monfloor}[1]{\left\lfloor #1\right\rfloor}
...
In this article, the numerical cost and the accuracy of Implicit Particle
Filter (IPF) are quantified, using Burgers' equations and shallow water
equations as scenarios. Identifying the uncertities in a data assimilation
problem are specifically addressed.
...
%Further developments are considered
...
Indeed, IPF have been successfully used for data assimilation,
\cite{Chorin2010}, system identification, \cite{Morzfeld2012}, and parameter
estimation. It is particularly relevant for multimodal PDF; it provides an
unbiased estimate of the solutions, \citep{Atkins2013}. The method relies on
a few, carefully-chosen particles for the quantification of the conditional
pdf of the system. Identifying these particles is done through a (costly)
minimization, ensuring that each particle is associated with a
high-probablility area of the PDF. The moments of the estimated pdf, as in
standard MCMC methods, might be used for quantifying the uncertainties.
...
Let's consider the state vector $\point$ of a system of interest, evolving
on the $\Nphaz$ manifold $\Ephaz $. This field is modeled by the set of
equations $\fdyn$:
...
\label{eq-lag}
\end{equation}
where $^{\dagger}$ is the transpose operator. As $\fdyn$ and $g$ are zero by
construction, $\lambda$ and $\mu$ can be designed for simplifying the
computations.
...
\[
  \int_0^T\lambda^{\dagger}\partial_{\dot{\point}}
  \fdyn(\point,\dot{\point},q,t)d_q \dot{\point} dt =
  [\lambda^{\dagger}\partial_{\dot{\point}} \fdyn(\point,\dot{\point},q,t) d_q
  \point ]_0^T - \int_0^T \mya{\dot{\lambda}^{\dagger}\partial_{\dot{\point}}
    \fdyn(u,\dot{\point},q,t) +
    \lambda^{\dagger}d_t\partial_{\dot{\point}}\fdyn(\point,\dot{\point},q,t)}d_q
  \point dt
  .
\]
The term associated with $d_{\dot{\point}}$ can now be replaced in
Eq.~\eqref{eq-deriv_lag}. Ordering terms in this equation leads to
\begin{equation}
\begin{split}
...
& \qquad \mya{\mu^{\dagger} \partial_{\point} g(\point,q) -
  \lambda^{\dagger}\partial_{\dot{\point}} \fdyn(u,\dot{\point},q,t) }|_0
dq_{\point(0)}  + \mu^{\dagger} \partial_q g(\point(0),q) +
\mya{\lambda^{\dagger}\partial_{\dot{\point}}
  \fdyn(\point,\dot{\point},q,t)}|_T d_q \point(T)
.
\end{split}
\label{eq-deriv_ord}
\end{equation}
...
$$
integrated in backward time. Finally, the Lagrange parameter $\mu$ is set so
it nullifies the component associated with $dq_{\point(0)}$:
...
where $\point$ evolves on an $\Nphaz$-dimensional manifold $\Ephaz$. We
consider that it has been spatially discretized. The field $\point$ can
hence be modeled by $\hat{\point}$ through a set of ordinary differential
equations
\[
  \fdyn(\hat{\point},\dot{\hat{\point}},q,t)
  :=
  \dot{\hat{\point}}
  +
  L(\hat{\point},q,t)
...
\begin{figure}[t]
\center
\includegraphics[height=.25\linewidth]{./fig/min.eps}
...
The parameters $q$ are actually the initial conditions. Hence, $n_q$ is
equal to $\Nphaz$. As a matter of fact, $g$ is
